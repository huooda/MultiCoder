...
import { AgentMessage, AgentType } from "../agents/types"
...
export class AgentChatViewProvider extends ClineProvider {
  private static readonly viewType = 'cline.agentChatView'
   ...
   */
  protected async handleAgentWebviewMessage(message: any): Promise<void> {
    // 处理智能体相关的消息
   ...
   * 注册智能体消息处理器
   */
  registerAgentMessageHandlers(): void {
    // 处理智能体消息
    const messageBus = require('../coordination/MessageBus').MessageBus.getInstance()
    messageBus.onMessage((message: AgentMessage) => {
