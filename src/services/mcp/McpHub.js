...
export class McpHub {
    ...
    getMode() {
        return vscode.workspace.getConfiguration("cline.mcp").get("mode", "full");
    }
  ...
  "mcpServers": {
    ...
    async toggleToolAutoApprove(serverName, toolName, shouldAllow) {
        try {
            const settingsPath = await this.getMcpSettingsFilePath();
            const content = await fs.readFile(settingsPath, "utf-8");
            const config = JSON.parse(content);
    ...
    async deleteServer(serverName) {
        try {
            ...
            vscode.window.showErrorMessage(`Failed to delete MCP server: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
