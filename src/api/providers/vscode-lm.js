...
export class VsCodeLmHandler {
    ...
    constructor(options) {
        ...
        try {
            ...
            this.dispose();
            throw new Error(`Cline <Language Model API>: Failed to initialize handler: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
                ...
                sendRequest: async (messages, options, token) => {
                    ...
                    return {
                        stream: (async function* () {
                            yield new vscode.LanguageModelTextPart("Language model functionality is limited. Please check VS Code configuration.");
                        })(),
                        text: (async function* () {
    ...
    async calculateTotalInputTokens(systemPrompt, vsCodeLmMessages) {
        ...
        return systemTokens + messageTokens.reduce((sum, tokens) => sum + tokens, 0);
    }
    ensureCleanState() {
        if (this.currentRequestCancellation) {
            this.currentRequestCancellation.cancel();
    ...
    async *createMessage(systemPrompt, messages) {
        ...
        const totalInputTokens = await this.calculateTotalInputTokens(systemPrompt, vsCodeLmMessages);
