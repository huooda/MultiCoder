...
class CheckpointTracker {
    ...
    lastCheckpointHash;
    constructor(provider, taskId, cwd) {
        this.providerRef = new WeakRef(provider);
        this.taskId = taskId;
        this.cwd = cwd;
    }
    static async create(taskId, provider) {
        try {
            if (!provider) {
                throw new Error("Provider is required to create a checkpoint tracker");
            }
            // Check if checkpoints are disabled in VS Code settings
            const enableCheckpoints = vscode.workspace.getConfiguration("cline").get("enableCheckpoints") ?? true;
    ...
    static async doesShadowGitExist(taskId, provider) {
    ...
    async initShadowGit() {
        ...
        if (await fileExistsAtPath(gitPath)) {
            ...
            await fs.writeFile(excludesPath, [
...
//# sourceMappingURL=CheckpointTracker-old.js.map