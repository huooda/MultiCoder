...
export class DiffViewProvider {
    ...
    async saveChanges() {
        ...
        */
        ...
        const newProblems = diagnosticsToProblemsString(getNewDiagnostics(this.preDiagnostics, postDiagnostics), [
    ...
    async openDiffEditor() {
        ...
        return new Promise((resolve, reject) => {
            ...
            vscode.commands.executeCommand("vscode.diff", vscode.Uri.parse(`${DIFF_VIEW_URI_SCHEME}:${fileName}`).with({
                query: Buffer.from(this.originalContent ?? "").toString("base64"),
            }), uri, `${fileName}: ${fileExists ? "Original ↔ Cline's Changes" : "New File"} (Editable)`);
