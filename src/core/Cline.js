 ...
 * @returns 处理后的参数值
 */
function removeClosingTag(paramName, value) {
    if (!value)
        return "";
    // 移除可能的XML闭合标签
...
export class Cline {
    ...
    async restoreCheckpoint(messageTs, restoreType) {
    ...
    async presentMultifileDiff(messageTs, seeNewChangesSinceLastTaskCompletion) {
        ...
        try {
            if (seeNewChangesSinceLastTaskCompletion) {
                ...
                const lastTaskCompletedMessageCheckpointHash = findLast(this.clineMessages.slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash; // ask is only used to relinquish control, its the last say we care about
                ...
                const firstCheckpointMessageCheckpointHash = this.clineMessages.find((m) => m.say === "checkpoint_created")?.lastCheckpointHash;
    ...
    async sayAndCreateMissingParamError(toolName, paramName, relPath) {
        await this.say("error", `Cline tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""} without value for required parameter '${paramName}'. Retrying...`);
    ...
    async resumeTaskFromHistory() {
        ...
        const lastRelevantMessageIndex = findLastIndex(modifiedClineMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"));
        ...
        const existingApiConversationHistory = await this.getSavedApiConversationHistory();
        ...
        newUserContent.push({
            type: "text",
            text: `[TASK RESUMPTION] ${this.chatSettings?.mode === "plan"
    ...
    async saveCheckpoint(isAttemptCompletionMessage = false) {
        ...
        if (!isAttemptCompletionMessage) {
            ...
            const lastCompletionResultMessage = findLast(this.clineMessages, (m) => m.say === "completion_result" || m.ask === "completion_result");
    ...
    async executeCommandTool(command) {
        ...
        if (userFeedback) {
            ...
            return [
                true,
                formatResponse.toolResult(`Command is still running in the user's terminal.${result.length > 0 ? `\nHere's the output so far:\n${result}` : ""}\n\nThe user provided the following feedback:\n<feedback>\n${userFeedback.text}\n</feedback>`, userFeedback.images),
            ];
        }
        if (completed) {
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const preferredLanguage = getLanguageKey(vscode.workspace.getConfiguration("cline").get("preferredLanguage"));
        ...
        if (await fileExistsAtPath(clineRulesFilePath)) {
            if (await isDirectory(clineRulesFilePath)) {
                try {
                    ...
                    const ruleFileContent = await Promise.all(ruleFiles.map(async (file) => {
        ...
        if (settingsCustomInstructions ||
            ...
            const allCustomInstructions = addUserInstructions(settingsCustomInstructions || '', clineRulesFileInstructions, clineIgnoreInstructions, preferredLanguageInstructions);
        ...
        if (previousApiReqIndex >= 0) {
            ...
            if (previousRequest && previousRequest.text) {
                ...
                if (totalTokens >= maxAllowedSize) {
                    ...
                    this.conversationHistoryDeletedRange = getNextTruncationRange(this.apiConversationHistory, this.conversationHistoryDeletedRange, keep);
        ...
        const truncatedConversationHistory = getTruncatedMessages(this.apiConversationHistory, this.conversationHistoryDeletedRange);
            ...
            case "tool_use":
                ...
                if (block.name === "create_coder_agent") {
                    try {
                        ...
                        // 询问用户是否批准创建代码智能体
                        const approvalMessage = `是否允许创建代码智能体来处理任务: "${task_description.substring(0, 50)}${task_description.length > 50 ? "..." : ""}"?`;
                        ...
                        if (approved) {
                            try {
                                ...
                                // 处理AgentManager相关错误
                                pushToolResult(formatResponse.toolError(`创建代码智能体失败: ${error instanceof Error ? error.message : String(error)}`));
                            }
                        }
                    }
                    catch (error) {
                        ...
                        try {
                            ...
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "tool");
                                ...
                                if (userEdits) {
                                    ...
                                    pushToolResult(`The user made the following updates to your content:\n\n${userEdits}\n\n` +
                                        (autoFormattingEdits
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to read ${path.basename(absolutePath)}`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "tool");
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to view directory ${path.basename(absolutePath)}/`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "tool");
                                    const didApprove = await askApproval("tool", completeMessage);
                                    if (!didApprove) {
                                        telemetryService.captureToolUsage(this.taskId, block.name, false, false);
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to view source code definitions in ${path.basename(absolutePath)}/`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "tool");
                                    const didApprove = await askApproval("tool", completeMessage);
                                    if (!didApprove) {
                                        telemetryService.captureToolUsage(this.taskId, block.name, false, false);
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to search files in ${path.basename(absolutePath)}/`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "tool");
                                    const didApprove = await askApproval("tool", completeMessage);
                                    if (!didApprove) {
                                        telemetryService.captureToolUsage(this.taskId, block.name, false, false);
                            ...
                            if (block.partial) {
                                ...
                                if (action === "launch") {
                                    ...
                                    if (this.shouldAutoApproveTool(block.name)) {
                                        ...
                                        showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to use a browser and launch ${url}`);
                                        this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch");
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (ignoredFileAttemptedToAccess) {
                                    ...
                                    pushToolResult(formatResponse.toolError(formatResponse.clineIgnoreError(ignoredFileAttemptedToAccess)));
                                    break;
                                }
                                ...
                                if (!requiresApproval && this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to execute a command: ${command}`);
                                    // this.removeLastPartialMessageIfExistsWithType("say", "command")
                                    const didApprove = await askApproval("command", command +
                                        `${this.shouldAutoApproveTool(block.name) && requiresApproval ? COMMAND_REQ_APP_STRING : ""}`);
                                    if (!didApprove) {
                                        break;
                                    }
                                }
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to use ${tool_name} on ${server_name}`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server");
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (this.shouldAutoApproveTool(block.name)) {
                                    ...
                                    showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to access ${uri} on ${server_name}`);
                                    this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server");
                                    const didApprove = await askApproval("use_mcp_server", completeMessage);
                                    if (!didApprove) {
                                        break;
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false, isNewTask = false) {
        if (this.abort) {
            throw new Error("Cline instance aborted");
        }
        if (this.consecutiveMistakeCount >= 3) {
            ...
            const { response, text, images } = await this.ask("mistake_limit_reached", this.api.getModel().id.includes("claude")
        ...
        if (this.autoApprovalSettings.enabled &&
            ...
            await this.ask("auto_approval_max_req_reached", `Cline has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`);
        ...
        if (!this.checkpointTracker && !this.checkpointTrackerErrorMessage) {
            try {
                this.checkpointTracker = await pTimeout(CheckpointTracker.create(this.taskId, this.providerRef.deref()?.context.globalStorageUri.fsPath), {
                    milliseconds: 15_000,
                    message: "Checkpoints taking too long to initialize. Consider re-opening Cline in a project that uses git, or disabling checkpoints.",
                });
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : "Unknown error";
