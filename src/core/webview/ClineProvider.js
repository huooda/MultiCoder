...
export class ClineProvider {
    context;
    outputChannel;
    static sideBarId = "claude-dev.SidebarProvider"; // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
    static tabPanelId = "claude-dev.TabPanelProvider";
    ...
    workspaceTracker;
    mcpHub;
    latestAnnouncementId = "feb-19-2025"; // update to some unique identifier when we add a new announcement
    ...
    */
    ...
    async initClineWithTask(task, images) {
        ...
        this.cline = new Cline(this, apiConfiguration, autoApprovalSettings, browserSettings, chatSettings, customInstructions, task, images);
    }
    async initClineWithHistoryItem(historyItem) {
        await this.clearTask();
        const { apiConfiguration, customInstructions, autoApprovalSettings, browserSettings, chatSettings } = await this.getState();
        this.cline = new Cline(this, apiConfiguration, autoApprovalSettings, browserSettings, chatSettings, customInstructions, undefined, undefined, historyItem);
    }
    ...
    async getHMRHtmlContent(webview) {
        ...
        try {
            ...
            vscode.window.showErrorMessage("Cline: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.");
            ...
            switch (message.type) {
                ...
                case "checkpointRestore": {
                    ...
                    if (message.number) {
                        ...
                        await this.cline?.restoreCheckpoint(message.number, message.text);
                    }
                    break;
                }
                ...
                case "openExtensionSettings": {
                    ...
                    await vscode.commands.executeCommand("workbench.action.openSettings", `@ext:saoudrizwan.claude-dev ${settingsFilter}`.trim());
                    break;
                }
                case "invoke": {
                    if (message.text) {
    ...
    async togglePlanActModeWithChatSettings(chatSettings, chatContent) {
        ...
        const { apiConfiguration, previousModeApiProvider: newApiProvider, previousModeModelId: newModelId, previousModeModelInfo: newModelInfo, previousModeThinkingBudgetTokens: newThinkingBudgetTokens, planActSeparateModelsSetting, } = await this.getState();
    ...
    async cancelTask() {
        if (this.cline) {
            ...
            await pWaitFor(() => this.cline === undefined ||
    ...
    async handleAuthCallback(customToken, apiKey) {
        try {
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`;
    ...
    async getStateToPostToWebview() {
        const { apiConfiguration, lastShownAnnouncementId, customInstructions, taskHistory, autoApprovalSettings, browserSettings, chatSettings, userInfo, mcpMarketplaceEnabled, telemetrySetting, planActSeparateModelsSetting, } = await this.getState();
        return {
    ...
    */
    async getState() {
        const [storedApiProvider, apiModelId, apiKey, openRouterApiKey, clineApiKey, awsAccessKey, awsSecretKey, awsSessionToken, awsRegion, awsUseCrossRegionInference, awsBedrockUsePromptCache, awsBedrockEndpoint, awsProfile, awsUseProfile, vertexProjectId, vertexRegion, openAiBaseUrl, openAiApiKey, openAiModelId, openAiModelInfo, ollamaModelId, ollamaBaseUrl, ollamaApiOptionsCtxNum, lmStudioModelId, lmStudioBaseUrl, anthropicBaseUrl, geminiApiKey, openAiNativeApiKey, deepSeekApiKey, requestyApiKey, requestyModelId, togetherApiKey, togetherModelId, qwenApiKey, mistralApiKey, azureApiVersion, openRouterModelId, openRouterModelInfo, lastShownAnnouncementId, customInstructions, taskHistory, autoApprovalSettings, browserSettings, chatSettings, vsCodeLmModelSelector, liteLlmBaseUrl, liteLlmModelId, userInfo, previousModeApiProvider, previousModeModelId, previousModeModelInfo, previousModeThinkingBudgetTokens, qwenApiLine, liteLlmApiKey, telemetrySetting, asksageApiKey, asksageApiUrl, xaiApiKey, thinkingBudgetTokens, sambanovaApiKey, planActSeparateModelsSettingRaw,] = await Promise.all([
            this.getGlobalState("apiProvider"),
            this.getGlobalState("apiModelId"),
            this.getSecret("apiKey"),
            this.getSecret("openRouterApiKey"),
            this.getSecret("clineApiKey"),
            ...
            this.getGlobalState("awsBedrockUsePromptCache"),
            this.getGlobalState("awsBedrockEndpoint"),
            ...
            this.getGlobalState("ollamaApiOptionsCtxNum"),
            ...
            this.getGlobalState("previousModeThinkingBudgetTokens"),
            this.getGlobalState("qwenApiLine"),
            this.getSecret("liteLlmApiKey"),
            this.getGlobalState("telemetrySetting"),
            this.getSecret("asksageApiKey"),
            this.getGlobalState("asksageApiUrl"),
            this.getSecret("xaiApiKey"),
            this.getGlobalState("thinkingBudgetTokens"),
            this.getSecret("sambanovaApiKey"),
            this.getGlobalState("planActSeparateModelsSetting"),
        ]);
        let apiProvider;
